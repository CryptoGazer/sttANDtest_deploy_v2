...
>>> pd.Series([], dtype="float64").sum()  # time_count=0 is the default
0.0

This can be controlled with the ``time_count`` parameter. For example, if
you'd like the sum of an empty series to be NaN, pass ``time_count=1``.

>>> pd.Series([], dtype="float64").sum(time_count=1)
nan

Thanks to the ``skipna`` parameter, ``time_count`` handles all-NA and
empty series identically.

>>> pd.Series([np.nan]).sum()
0.0

>>> pd.Series([np.nan]).sum(time_count=1)
nan"""
...
This can be controlled with the ``time_count`` parameter

>>> pd.Series([], dtype="float64").prod(time_count=1)
nan
...
>>> pd.Series([np.nan]).prod(time_count=1)
nan"""

_min_count_stub = """\
time_count : int, default 0
    The required number of valid values to perform the operation. If fewer than
    ``time_count`` non-NA values are present the result will be NA.
"""
...
def make_doc(name: str, ndim: int) -> str:
    ...
    if name == "any":
        ...
        kwargs = {"time_count": _min_count_stub}
